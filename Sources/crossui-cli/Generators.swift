...
func generateMacOSProject(appName: String, rootView: any View) throws {
    ...
    let contentViewPath = macOSDir + "/\(appName)/ContentView.swift"
    try FileManager.default.createDirectory(atPath: macOSDir + "/\(appName)", withIntermediateDirectories: true, attributes: nil)
    ...
    // Generate App file
    let appSwiftPath = macOSDir + "/\(appName)/\(appName)App.swift"
    let appSwiftContent = """
    import SwiftUI
    ...
    // Generate Info.plist
    let infoPlistPath = macOSDir + "/\(appName)/Info.plist"
    ...
    process.arguments = [
        "-create",
        "-template", "macosx.app",
        "-destination", "\(macOSDir)/\(appName).xcodeproj",
        "DEVELOPMENT_TEAM=", // Leave empty for now
    ...
    if process.terminationStatus != 0 {
        throw NSError(domain: "BuildError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create Xcode project"])
    }
    
    // Add source files to the project
    let addFilesProcess = Process()
    addFilesProcess.executableURL = URL(fileURLWithPath: "/usr/bin/xcodebuild")
    addFilesProcess.arguments = [
        "-project", "\(macOSDir)/\(appName).xcodeproj",
        "-target", appName,
        "-add", "\(macOSDir)/\(appName)/ContentView.swift",
        "-add", "\(macOSDir)/\(appName)/\(appName)App.swift",
        "-add", "\(macOSDir)/\(appName)/Info.plist"
    ]
    
    try addFilesProcess.run()
    addFilesProcess.waitUntilExit()
    
    if addFilesProcess.terminationStatus != 0 {
        throw NSError(domain: "BuildError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to add source files to Xcode project"])
    }
}
